  ...
  <div class="PrefixMan">
    <div class="Group">
      <div class="GroupTitle" @click="prefixListShow=!prefixListShow">
        用户组信息
        <i class="titleIcon fa" :class="[prefixListShow == true ? 'fa-chevron-up': 'fa-chevron-down']"></i>
      ...
      <transition name="slide-fade">
        <div class="GroupItem" v-if="prefixListShow">
    ...
    <mt-popup v-model="prefixDelVisible" popup-transition="popup-fade" class="delGrpDiv">
      <div class="popupContainer">
        ...
        <form action="" @submit.prevent="submitPrefixDel">
          ...
          <div class="fGrp" style="text-align: right">
            <button @click="prefixDelVisible = false" style="margin-right: .06rem;">取消</button>
            <button class="modalBtn" type="submit" style="background-color: #3d81f1;color:#fff;">确定</button>
          </div>
        </form>
      </div>
  ...
  export default {
    ...
    methods:{
      ...
      addLevel(level){
        ...
        that.savePrefixConfig(that.addNewUserPillbox(level,function(user,data){
