<template>
  <div class="UserMan">
    <div class="Group" style="height:100%">
      ...
      <mt-popup v-model="AccountShow" position="right" popup-transition="popup-fade" class="rightPop">
        <div class="GroupItem" v-if="AccountShow"  style="height:100%">
          <div class="popTitle">
            <div class="back">
              <div  @click="AccountShow=false" class="popTitleBack">
    ...
    <mt-popup v-model="deviceAuthourityVisible" position="right" popup-transition="popup-slide" class="wholePagePop devAuthority">
      <div class="page-navbar">
        ...
        <mt-navbar v-model="active">
          ...
          <mt-tab-item id="2">虚拟服务器</mt-tab-item>
          <mt-tab-item id="3">实体接收机</mt-tab-item>
        </mt-navbar>
        <mt-tab-container v-model="active" style="padding:2px 5px;">
          ...
          <mt-tab-container-item id="2" style="overflow-x:auto">
            <table>
              <template v-for="(item,i) in VirRcvRightsList">
                <tr>
          ...
          <mt-tab-container-item id="3" style="overflow-x:auto">
            <table>
  ...
  export default {
    ...
    methods:{
      ...
      showDevAuthority(item){
        ...
        this.getVirRcvAuthourData(item.id);
      },
      ...
      getVirRcvAuthourData(userId){
        var that = this;
        this.$axios({
          method: 'post',
          url:"/page/users/users.php",
          data:this.$qs.stringify({
            getUserVirRcvList: true,
            userId: userId,
          }),
          Api:"getUserVirRcvList",
          AppId:"android",
          UserId:that.user.id
        })
        .then(function (response) {
          ...
          if(res.res.success){
            ...
            that.VirRcvRightsList.push({"rcv_name": "别名",
                                  "rcv_sn": "序列号",
                                  "user_id": "用户",
                                  "prefix": "组",
                                  "rcv_model": "型号"}) 
            that.VirRcvRightsList = that.VirRcvRightsList.concat(res.data);
          }else{
            that.VirRcvRightsList = [];
          }
        })
        .catch(function (error) {
          console.log(error)
      ...
      getRcvAuthourData(userId){
        ...
        this.$axios({
          ...
          url:"/page/users/users.php",
          data:this.$qs.stringify({
            getUserPraRcvList: true,
            userId: userId,
          }),
          Api:"getUserPraRcvList",
          AppId:"android",
          UserId:that.user.id
        })
        .then(function (response) {
          ...
          if(res.res.success){
            //that.rcvRightsList = res.data;
            that.rcvRightsList.push({"rcv_name": "别名",
                                  "rcv_sn": "序列号",
                                  "user_id": "用户",
                                  "prefix": "组",
                                  "rcv_model": "型号"}) 
